...
export class BookEditComponent implements OnInit {
  ...
  protected loadBook(): void {
    ...
    this.service.getOne(id)
      .subscribe(
        result => {
          // @ts-ignore
          const book = result.data.findOneBook;
          console.log(book);
          const formItem = {};
    ...
    this.service.add(this.editForm.value)
      .subscribe(
        result => {
          ...
          // @ts-ignore
          this.router.navigate(showUrl(BOOK_URL, result.data.createBook.id.toString()));
        },
        error => {
  ...
  protected update(): void {
    const id = +getParam(this.route, 'id');
    this.service.update(id, this.editForm.value)
      .subscribe(
        result => {
          this.service.changeMessage(`Book was successfully updated.`);
          console.log(result.data)
          // @ts-ignore
          this.router.navigate(showUrl(BOOK_URL, result.data.updateBook.id));
        },
        error => {
          this.service.changeMessage(error.error ? error.error.message : error.message);
        }
